...
# Keep the CLI clean: Telethon/asyncio connection noise is hidden from the terminal.
# Real application errors are still handled by the existing exception handlers.
...
SESSION_NAME = os.path.join(BASE_DIR, "telegram_cli")
DB_FILE = os.path.join(BASE_DIR, "telegram_cli.db")
...
AUTO_DOWNLOAD_MAX_SIZE = 25 * 1024 * 1024
...
AUTO_DOWNLOAD_CONCURRENCY = 6

DOWNLOAD_QUEUE_MAXSIZE = 2000

DOWNLOAD_POLL_INTERVAL = 0.01
AUTO_DOWNLOAD_TIMEOUT = 180
...
def message_ref(chat_id, telegram_id):
    """Stable, human-facing message identifier.

    Telegram message IDs are only unique inside a chat, so the stable
    identifier is the pair chat_id + telegram_id. This never depends on
    SQLite AUTOINCREMENT/local_id.
    """
    return f"{int(chat_id)}:{int(telegram_id)}"


def get_message_by_telegram_id(chat_id, telegram_id):
    return db.execute(
        """
        SELECT
            local_id, chat_id, telegram_id, sender_id, sender_name,
            text, outgoing, date, media_path, media_type, media_name
        FROM messages
        WHERE chat_id=? AND telegram_id=?
        LIMIT 1
        """,
        (int(chat_id), int(telegram_id))
    ).fetchone()


def resolve_message_ref(ref, chat_id=None):
    """Resolve a stable message reference to the internal local_id.

    Accepted forms:
      chat_id:telegram_id   (preferred/global)
      telegram_id          (inside the current chat)
    """
    ref = str(ref).strip()
    if not ref:
        return None
    ...
    try:
        if ":" in ref:
            chat_part, tg_part = ref.split(":", 1)
            row = get_message_by_telegram_id(
                int(chat_part), int(tg_part)
            )
            ...
            # Outside a chat, a bare Telegram ID is only accepted when it
            # identifies exactly one stored message.
            rows = db.execute(
                """
                SELECT local_id
                FROM messages
...
def detect_media_type(message):
    """Return a stable media category for photos, videos, and audio/voice."""
    ...
    try:
        ...
        # Telethon convenience attributes are more reliable than MIME alone.
        ...
        if document:
            ...
            # Some Telegram files have missing/incorrect MIME types.
            ...
            if getattr(document, "thumbs", None) and mime == "":
                # Do not guess an arbitrary type from a thumbnail alone.
                pass
...
async def download_message_media(
    ...
    try:
        if queued_message is not None:
            ...
            message = await client.get_messages(entity, ids=telegram_id)
        ...
        if automatic:
            downloaded = await asyncio.wait_for(
                message.download_media(file=destination),
                timeout=AUTO_DOWNLOAD_TIMEOUT
            )
...
async def queue_auto_download(local_id, message=None):
    ...
    try:
        download_queue.put_nowait((local_id, message))
    ...
    while running:
        try:
            ...
            if isinstance(item, tuple):
                local_id, queued_message = item
            else:
                local_id, queued_message = item, None
...
async def save_telegram_message(
    ...
    name = sender_name_from_entity(sender_obj) if sender_obj else "Unknown"
    ...
    # Automatically download every supported image/video/audio <= 25 MB.
    # This also covers media discovered during background/history sync.
    if can_auto_download(message):
        await queue_auto_download(
            local_id,
            message
        )
...
class ViewerHandler(
    ...
    def do_GET(self):
        ...
        if path.startswith("/message/"):
            ...
            # Preferred: /message/<chat_id>/<telegram_id>
            if len(parts) == 3:
                try:
                    chat_id = int(parts[1])
                    ...
                    self.send_error(400)
                    return
                row = get_message_by_telegram_id(chat_id, telegram_id)
            # Backward-compatible: /message/<local_id>
            # Kept only for old bookmarks; new links never use it.
            elif len(parts) == 2:
                try:
                    legacy_local_id = int(parts[1])
                except ValueError:
                    self.send_error(400)
                    return
                row = get_message(legacy_local_id)
            else:
                self.send_error(400)
                return
            ...
            self.send_html(build_message_html(row))
            return
...
def build_message_html(row):
    ...
    if media_path:
        ...
        if media_type in AUTO_DOWNLOAD_MEDIA_TYPES:
            content += (
                '<div class="warning">'
                'This media is larger than 25 MB '
...
async def show_new_messages():
    ...
    for (
        ...
        print(
            color(
                f"[{message_ref(chat_id, telegram_id)}]",
                C.BRIGHT_YELLOW,
                C.BOLD
            )
            + " "
    ...
    print(
        cyan("opennew <chat_id:telegram_id>")
...
async def open_new_message(
    ...
    local_id = resolve_message_ref(message_id)
    row = get_message(local_id) if local_id is not None else None
...
async def reply_message(
    ...
    local_id = resolve_message_ref(message_id, current_chat["id"])
    row = get_message(local_id) if local_id is not None else None
...
async def cat_message(message_id):
    chat_id = current_chat["id"] if current_chat else None
    local_id = resolve_message_ref(message_id, chat_id)
    row = get_message(local_id) if local_id is not None else None
    ...
    print(
        muted("Message ID")
        + " : "
        + cyan(message_ref(chat_id, telegram_id))
    )
    ...
    if media_path:
        ...
        print(
            ...
            + " : "
            + cyan(f"d {message_ref(chat_id, telegram_id)}")
        )
...
async def d_command(local_id):
    ...
    row = get_message(local_id)
    display_id = (
        message_ref(row[1], row[2])
        if row else str(local_id)
    )

    header(
        f"DOWNLOAD  │  MESSAGE {display_id}"
    )
...
async def command_loop():
    ...
    while running:
        try:
                ...
                if cmd == "opennew":
                    if len(parts) < 2:
                        print(
                            "\n"
                            + warning(
                                "Usage: opennew <telegram_id> or <chat_id:telegram_id>"
                            )
                        )
                    ...
                    message_id = parts[1]
                    await open_new_message(message_id)
            ...
            if cmd == "back":
                # IMPORTANT: never call Telegram/network APIs on `back`.
                # Re-render from the already cached local data immediately.
                mark_chat_read(current_chat["id"])
                current_chat = None
                await redraw()
                continue
            ...
            if cmd == "reply":
                ...
                message_id = parts[1]
                await reply_message(message_id, parts[2])
